...
--
-- Each input node carries a `config.default` payload, so a fresh `Run All`
-- with no body uses the seeded sample (Release 1.2 / the toy XML feed) and
-- the pipeline does real work. A POST /api/pipelines/:id/run with an
-- explicit body still wins, per the runtime's input-node merge:
--   outputs = { ...config.default, ...runPayload }
--
-- HOW TO RUN (with a custom payload — Run All uses the defaults above)
--
