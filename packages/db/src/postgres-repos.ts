  ...
  implements TenantGitConfigRepository
{
  ...
  async get(tenantId: UUID): Promise<TenantGitConfigRow | undefined> {
    const r = await this.pool.query<Record<string, unknown>>(
      `SELECT * FROM tenant_git_configs WHERE tenant_id = $1`,
      [tenantId]
    );
    return r.rows[0] ? rowFromDb<TenantGitConfigRow>(r.rows[0]) : undefined;
  }
  ...
  async upsert(row: TenantGitConfigRow): Promise<TenantGitConfigRow> {
    ...
    return rowFromDb<TenantGitConfigRow>(r.rows[0]);
  }
  ...
  async listDue(nowIso: string): Promise<TenantGitConfigRow[]> {
    ...
    return r.rows.map((row) => rowFromDb<TenantGitConfigRow>(row));
  }
...
export class PostgresDatasetRepository
  extends PostgresCrudRepository<DatasetRow>
...
export class PostgresDatasetVersionRepository
  extends PostgresCrudRepository<DatasetVersionRow>
  implements DatasetVersionRepository
{
  ...
  async listByDataset(datasetId: string): Promise<DatasetVersionRow[]> {
    return this.queryRows(
...
export class PostgresDatasetAliasRepository implements DatasetAliasRepository {
  private pool: PoolLike;
  constructor(pool: PoolLike) {
    this.pool = pool;
  }
  async upsert(row: DatasetAliasRow): Promise<DatasetAliasRow> {
    ...
    return rowFromDb<DatasetAliasRow>(result.rows[0]);
  }
  async resolve(
    datasetId: string,
    alias: string
  ): Promise<DatasetAliasRow | undefined> {
    const result = await this.pool.query<Record<string, unknown>>(
      `SELECT * FROM dataset_aliases WHERE dataset_id = $1 AND alias = $2`,
      [datasetId, alias]
    );
    return result.rows[0] ? rowFromDb<DatasetAliasRow>(result.rows[0]) : undefined;
  }
  async listByDataset(datasetId: string): Promise<DatasetAliasRow[]> {
    const result = await this.pool.query<Record<string, unknown>>(
      `SELECT * FROM dataset_aliases WHERE dataset_id = $1 ORDER BY alias`,
      [datasetId]
    );
    return result.rows.map((r) => rowFromDb<DatasetAliasRow>(r));
  }
