...
export class InMemoryDatasetRepository implements DatasetRepository {
  protected rows = new Map<string, DatasetRow>();

  async create(row: DatasetRow): Promise<DatasetRow> {
    if (this.rows.has(row.id)) {
  ...
  async get(id: string): Promise<DatasetRow | undefined> {
    const row = this.rows.get(id);
    return row ? structuredClone(row) : undefined;
  }

  async require(id: string): Promise<DatasetRow> {
    const row = await this.get(id);
    if (!row) throw new NotFoundError("dataset", id);
    return row;
  }

  async update(id: string, patch: Partial<DatasetRow>): Promise<DatasetRow> {
    const existing = this.rows.get(id);
...
{
  protected rows = new Map<string, DatasetVersionRow>();

  async create(row: DatasetVersionRow): Promise<DatasetVersionRow> {
    if (this.rows.has(row.id)) {
  ...
  async get(id: string): Promise<DatasetVersionRow | undefined> {
    const row = this.rows.get(id);
    return row ? structuredClone(row) : undefined;
  }
  ...
  async update(
    id: string,
    patch: Partial<DatasetVersionRow>
...
export class InMemoryDatasetAliasRepository
  implements DatasetAliasRepository
{
  protected rows = new Map<string, DatasetAliasRow>();
