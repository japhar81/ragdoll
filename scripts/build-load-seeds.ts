...
// Deterministic UUID for the "Load Tests" folder every load-* pipeline lives
// in. The matching `00000000-7777-7777-7777-...` prefix on the pipeline rows
// makes the relationship obvious in a DB dump; the trailing literal
// `f01dferff100` (folder, load) marks the folder row distinctly.
const LOAD_FOLDER_ID = "00000000-7777-7777-7777-f01df01df100";
const LOAD_FOLDER_NAME = "Load Tests";
...
--
-- All load-* pipelines live in a dedicated "${LOAD_FOLDER_NAME}" folder so
...
function renderFolderBlock(slugs: string[]): string {
  const slugList = slugs.map((s) => `  '${s}'`).join(",\n");
  return `-- ------------------------------ ${LOAD_FOLDER_NAME} folder ------------------------------

INSERT INTO pipeline_folders (id, parent_id, name)
VALUES ('${LOAD_FOLDER_ID}', NULL, '${LOAD_FOLDER_NAME}')
...
UPDATE pipelines
SET folder_id = '${LOAD_FOLDER_ID}'
WHERE folder_id IS DISTINCT FROM '${LOAD_FOLDER_ID}'
  AND slug IN (
${slugList}
);
`;
...
async function main(): Promise<void> {
  ...
  const folderBlock = renderFolderBlock(built.map((b) => b.slug));
