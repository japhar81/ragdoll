...
| `fatal: detected dubious ownership in repository at '/opt/ragdoll-plugins/.git'` | The repo dir is owned by a different user than the container's runtime UID (git's CVE-2022-24765 guard) — e.g. a bind-mounted source, or the sidecar running as a different UID than the api/worker. Classically shows up on a **sidecar** (`host: sidecar`) source while the same-URL **worker** (`host: worker`) source works, because the two run as different UIDs. Fixed in both loaders (`0.2.x+`): they run git with `safe.directory=*` supplied through `GIT_CONFIG_GLOBAL` pointing at a temp config (which also `[include]`s any operator global config). A *global* config file — not scoped `GIT_CONFIG_*` env vars — is required because a `file://` remote forks a separate `git-upload-pack` child that re-reads global config and ignores the inline vars. Upgrade the api/worker **and** python-plugins sidecar images (the sidecar image is **excluded** from `make refresh` — rebuild it explicitly with `docker compose up -d --build python-plugins`); no `chown` or per-directory config needed. |
